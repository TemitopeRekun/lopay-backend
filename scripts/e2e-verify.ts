...
async function main() {
  ...
  console.log(
    `   (using ADMIN_EMAIL=${ADMIN_EMAIL}, pw len=${ADMIN_PW.length})`,
  );
  const admin = await signIn(ADMIN_EMAIL, ADMIN_PW);
  check(
    'admin sign-in 200',
    admin.status === 200,
    `got ${admin.status} body=${JSON.stringify(admin.j)?.slice(0, 200)}`,
  );
  if (!admin.token) {
  ...
  check(
    ...
    JSON.stringify(adminSess?.user?.role),
  );
  ...
  check(
    ...
    `status ${onboard.status} ${JSON.stringify(onboard.j)?.slice(0, 160)}`,
  );
  const schoolId = onboard.j?.school?.id;
  check('school created with id', !!schoolId);
  check(
    'owner user returned as SCHOOL_OWNER',
    onboard.j?.user?.role === 'SCHOOL_OWNER',
    JSON.stringify(onboard.j?.user?.role),
  );
  ...
  check(
    'owner session has schoolId (customSession)',
    ownerSess?.user?.schoolId === schoolId,
    `sess=${ownerSess?.user?.schoolId} school=${schoolId}`,
  );

  // ── 4. Owner creates a class fee (stored kobo, returned naira) ────────────
  console.log('4. Class fee');
  const fee = await api('POST', '/school-payments/fees', owner.token!, {
    className: 'Grade 1',
    feeAmount: 100000,
  });
  check(
    'create fee ok',
    fee.status === 200 || fee.status === 201,
    `status ${fee.status}`,
  );
  ...
  check(
    'fee returned in naira (100000)',
    grade1?.feeAmount === 100000,
    `got ${grade1?.feeAmount}`,
  );
  ...
  check(
    'parent sign-up ok',
    !!parent.token,
    `status ${parent.status} ${JSON.stringify(parent.j)?.slice(0, 120)}`,
  );
  const parentUserId = parent.j?.user?.id;
  check(
    'parent role = PARENT (not escalated)',
    parent.j?.user?.role === 'PARENT',
    JSON.stringify(parent.j?.user?.role),
  );
  ...
  check(
    'cannot self-assign SUPER_ADMIN at signup',
    evilJson?.user?.role !== 'SUPER_ADMIN',
    JSON.stringify(evilJson?.user?.role),
  );
  await prisma.user
    .deleteMany({ where: { email: 'e2e-evil@lopay.test' } })
    .catch(() => {});
  ...
  const parentRow = await prisma.parent.create({
    data: { userId: parentUserId, phoneNumber: '08020000000' },
  });
  ...
  const inst = await api(
    ...
    parent.token!,
    { enrollmentId: enrollment.id, amountPaid: 25000 },
  );
  check(
    'installment submitted',
    inst.status === 200 || inst.status === 201,
    `status ${inst.status} ${JSON.stringify(inst.j)?.slice(0, 160)}`,
  );
  check(
    'installment amount echoed as naira (25000)',
    inst.j?.amount === 25000 || inst.j?.amountPaid === 25000,
    `got ${inst.j?.amount}/${inst.j?.amountPaid}`,
  );
  ...
  const over = await api(
    ...
    '/enrollments/pay-installment',
    parent.token!,
    { enrollmentId: enrollment.id, amountPaid: 999999 },
  );
  check(
    'over-balance installment rejected (400)',
    over.status === 400,
    `got ${over.status}`,
  );
  ...
  const pendingPayment =
    (pending.j as any[])?.find?.((p) => p.enrollmentId === enrollment.id) ||
    (pending.j as any[])?.[0];
  check(
    'pending installment visible to owner',
    !!pendingPayment,
    `count ${(pending.j as any[])?.length}`,
  );
  check(
    'pending amount in naira (25000)',
    pendingPayment?.amount === 25000 || pendingPayment?.amountPaid === 25000,
    `got ${pendingPayment?.amount}/${pendingPayment?.amountPaid}`,
  );

  // ── 9. Owner confirms → balance recompute + audit ─────────────────────────
  const confirm = await api('POST', '/school-payments/confirm', owner.token!, {
    paymentId: pendingPayment?.id,
  });
  check(
    'confirm ok',
    confirm.status === 200 || confirm.status === 201,
    `status ${confirm.status} ${JSON.stringify(confirm.j)?.slice(0, 160)}`,
  );
  ...
  check(
    'my-children returns the child',
    !!myChild,
    `count ${(kids.j as any[])?.length}`,
  );
  check(
    'remaining balance recomputed to ₦50,000',
    myChild?.remainingBalance === 50000,
    `got ${myChild?.remainingBalance}`,
  );
  check(
    'paid amount = ₦25,000 (naira)',
    myChild?.paidAmount === 25000,
    `got ${myChild?.paidAmount}`,
  );
  check(
    'total fee = ₦100,000 (naira)',
    myChild?.totalFee === 100000,
    `got ${myChild?.totalFee}`,
  );
  ...
  check(
    'owner history shows confirmed ₦25,000',
    !!histItem,
    `items ${(history.j as any[])?.length}`,
  );
  const stats = await api('GET', '/school-payments/stats', owner.token!);
  check(
    'dashboard stats returned',
    stats.status === 200 && stats.j != null,
    `status ${stats.status}`,
  );
  ...
  check(
    'admin transactions include the payment',
    Array.isArray(txns.j) && (txns.j as any[]).some((t) => t.amount === 25000),
    `count ${(txns.j as any[])?.length}`,
  );
  const audit = await api('GET', '/audit-logs?take=20', admin.token, undefined);
  const auditItems = (audit.j as any)?.items;
  check(
    'audit log has PAYMENT_CONFIRMED',
    Array.isArray(auditItems) &&
      auditItems.some((a: any) => a.action === 'PAYMENT_CONFIRMED'),
    `shape ${JSON.stringify(audit.j)?.slice(0, 80)}`,
  );

  // ── 13. Authorization boundaries ──────────────────────────────────────────
  console.log('11. Authorization');
  const parentHitsAdmin = await api(
    'GET',
    '/admin/overview',
    parent.token!,
    undefined,
  ...
  check(
    'parent → admin route = 403',
    parentHitsAdmin.status === 403,
    `got ${parentHitsAdmin.status}`,
  );
